...
import { slowRoad, OptionalActivity, } from "@/data/slowRoad";
...
const SlowRoad: React.FC = () => {
  ...
  return (
    <>
      <SEO {...seoConfig.slowRoad} />
      
      {/* Floating Book Button */}
      <FloatingBookButton 
        tourName={slowRoad.name}
        price={tourData?.price}
      />
      
      <div className="space-y-10 p-6">
        <HeroHeader
          images={slowRoad.heroImages}
          title={slowRoad.name}
          subtitle={`${tourData?.durationDays || 12} days`}
        />

        <h1 className="text-3xl font-extrabold">{slowRoad.name}</h1>

        <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          <div className="lg:col-span-2 space-y-3">
            {slowRoad.summary[lang].map((para: string, idx: number) => (
        ...
        {slowRoad.mapImage && (
          <section>
            ...
            <ImageGallery images={[slowRoad.mapImage]} />
          </section>
        )}
        ...
        <section className="bg-gray-200 border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <ul className="list-disc pl-5 text-slate-700 space-y-1">
            {slowRoad.whatsIncluded?.[lang].map((item: string, idx: number) => (
              <li key={idx}>{item}</li>
            ))}
          </ul>
        </section>
