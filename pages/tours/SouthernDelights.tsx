...
import { southernDelights, OptionalActivity } from "@/data/southernDelights";
              ...
              <div>
                ...
                {activity.duration && (
                  <p className="text-sm text-slate-600 mt-1">
                    {t('tourPage.duration')}: {activity.duration}
                  </p>
                )}
              </div>
...
// ─── Main page component ──────────────────────────────────────────────────────
const SouthernDelights: React.FC = () => {
  ...
  const tourData = TOURS_DATA.find(t => t.id === 'southern-delights');
  ...
  return (
    <>
      ...
      <FloatingBookButton
        tourName={southernDelights.name}
      ...
      <div className="space-y-10 p-6">
        ...
        {/* Hero */}
        <HeroHeader
          images={southernDelights.heroImages}
          title={southernDelights.name}
          subtitle={`${tourData?.durationDays || 12} days`}
        />

        <h1 className="text-3xl font-extrabold">{southernDelights.name}</h1>
        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-2 space-y-3">
            {southernDelights.summary[lang].map((para, idx) => (
        ...
        <section>
          ...
          >
            {southernDelights.itinerary.map((day) => (
              <AccordionItem
                key={day.day}
                value={`day-${day.day}`}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-4">
                    ...
                    {day.includedActivities && (
                      <InfoItem icon={<CheckCircle2 />} title={t('tourPage.includedActivities')}>
                        <ul className="list-disc list-inside">
                          {day.includedActivities[lang].map((a, idx) => (
                            <li key={idx}>{a}</li>
                          ))}
                        </ul>
                      </InfoItem>
                    ...
                    {day.optionalActivities && day.optionalActivities[lang][0] !== "-" && (
                      <InfoItem icon={<PlusCircle />} title={t('tourPage.optionalActivities')}>
                        <ul className="list-disc list-inside">
                          {day.optionalActivities[lang].map((a, idx) => (
                            <li key={idx}>{a}</li>
                          ))}
                        </ul>
                      </InfoItem>
        ...
        {/* Tour Route Map */}
        {southernDelights.mapImage && (
          <section>
            <h2 className="text-2xl font-bold mb-4">{t('tourPage.tourRoute')}</h2>
            <ImageGallery images={[southernDelights.mapImage]} />
          </section>
        )}
        ...
        <section className="bg-gray-200 border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <ul className="list-disc pl-5 text-slate-700 space-y-1">
            {southernDelights.whatsIncluded?.[lang].map((item, idx) => (
              <li key={idx}>{item}</li>
            ))}
          </ul>
        </section>
...
export default SouthernDelights;